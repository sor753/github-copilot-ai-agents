...
- スタイリングは Tailwind CSS のユーティリティクラスで行う。独自 CSS ファイルは原則作成しない
- クラスの羅列が長くなる場合は `cn()` ヘルパー（`clsx` + `tailwind-merge`）でまとめる
- バリアントの出し分けは `cva`（class-variance-authority）を活用する
- `style` 属性によるインラインスタイルは動的な値（アニメーション・計算値）のみに限定する
- Tailwind のカスタムトークン（`tailwind.config` の `theme.extend`）でデザイントークンを一元管理する
...
- クラスコンポーネントは提案しない
- `any` 型は提案しない
- スタイリングは Tailwind CSS のユーティリティクラスで提案すること。独自 CSS は提案しない
- 長いクラス列は `cn()` ヘルパーでまとめること
- エラーハンドリングを省略しないこと
...
- **シンプルさを最優先する。** 洗練よりも温かみを重視する。ユーザーが日記を書く場であるため、圧迫感のない、落ち着いた雰囲気を作る
- **AI らしいデザインを避ける。** 以下はアンチパターンとして禁止する:
  - グラデーション背景・ガラスモーフィズム・ネオモーフィズム
  - `rounded-full` によるピル型ボタン
  - ヒーローセクションへの大型イラスト・Lottie アニメーション配置
  - 過剰なシャドウ（`shadow-xl` 以上）
  - 装飾目的のアイコンの多用
- **カラーは最小限に抑える。** 以下の3種類のみ使用する:
  - **ベース:** グレースケール（`stone-*` / `gray-*`）を中心に使用する。クールなブルーグレーより、やや温かみのある `stone` を優先する
  - **アクセント:** 1色のみ。落ち着いた土系・緑系のトーンを推奨する（例: `amber-700`・`teal-700`）
  - **セマンティック:** `danger`・`success` の2色のみ
...
`tailwind.config` でのセマンティックトークン定義例（温かみのあるパレット）：

```js
theme: {
  extend: {
    colors: {
      // アクセント: 1色のみ（落ち着いた土系・緑系を推奨）
      accent: { DEFAULT: '#b45309', hover: '#92400e' }, // amber-700 系
      // セマンティック: 2色のみ
